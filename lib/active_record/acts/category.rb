    ...
    module Category
      ...
      module ClassMethods        
        ...
        def acts_as_category(options = {})
          ...
          # after_create :build_descendants_ids_cache
          # after_update :build_descendants_ids_cache
          # after_destroy :build_descendants_ids_cache
          # 
      ...
      module InstanceMethods
        ...
        # Returns list of descendants, respecting permitted/hidden categories
       	...
       	def self_and_descendants_ids
       	  unless self.descendants_ids_cache
       	    self.build_descendants_ids_cache
     	   end
       	  [self.id] + Marshal.load(self.descendants_ids_cache)
       	end
        ...
        def build_descendants_ids_cache
          self.descendants_ids_cache = Marshal.dump(descendants_ids)
          self.save!
        end
        
        def refresh_descendants_ids_cache
          root.descendants.each do |category|
            category.build_descendants_ids_cache
          end        
        end
        ...
        def assign_position
          ...
          if self.read_attribute(parent_id_column).nil?
            self.update_attribute(position_column, self.class.roots(true).size + 1)
          else
            ...
            self.position_column = self.class.find(:all, :conditions => ["#{parent_id_column} = ?", self.read_attribute(parent_id_column)]).size + 1
          end
        end
